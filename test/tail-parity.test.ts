...
const PARITY_CASES: TailParityCase[] = [
  {
...
test('tail matches host GNU tail for the supported parity corpus', async function (t): Promise<void> {
  if (!(await hasOracleUtility('tail'))) {
    t.skip('host tail is not available');
    return;
  }
  ...
  for (const parityCase of PARITY_CASES) {
    await withOracleFixtureWorkspace(parityCase.files, async function (workspace): Promise<void> {
      ...
      const runtime = await runRuntimeTail(workspace, parityCase.args, parityCase.stdin);
...
async function runRuntimeTail(
  ...
  const { result } = await runCommand('tail', mapTextSliceArgs(workspace, args, true), {
    ctx,
